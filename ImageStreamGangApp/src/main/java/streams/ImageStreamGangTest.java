...
public class ImageStreamGangTest {
     ...
     */
    private static Map<String, List<Long>> mResultsMap = new HashMap<String, List<Long>>();
    ...
    private static void printTimingResults(Map<String, List<Long>> resultsMap) {
        ...
        int numberOfRuns = resultsMap.entrySet().iterator().next().getValue().size();
        
        // This local class is needed to make the Java compiler happy.
        class ResultMap extends TreeMap<Long, String> {}
//        
//        Supplier<ResultMap> sup = new Supplier () {
//            public ResultMap get() {
//                return new ResultMap();
//            }
//
//        };
//        ResultMap res = sup.get();
        ...
        //List<ResultMap> listOfMaps = Stream.generate((Supplier<ResultMap>) res).limit(numberOfRuns).collect(toList());
        //List<ResultMap> listOfMaps = Stream.generate(ResultMap::new).limit(numberOfRuns).collect(toList());
        
        for(int treeIndex = 0; treeIndex < numberOfRuns; treeIndex++){
        	for(Entry<String, List<Long>> entry : resultsMap.entrySet()){
                ...
                map.put(entry.getValue().get(treeIndex),
                        entry.getKey());
        	}
        }
//        
...
//                          }));
        ...
        for(int treeIndex = 0; treeIndex < numberOfRuns; treeIndex++){
        	...
        	for(Entry<Long, String> entry : listOfMaps.get(treeIndex).entrySet()){
