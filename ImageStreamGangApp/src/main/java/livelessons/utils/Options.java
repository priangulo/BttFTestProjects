...
import livelessons.utils.ExceptionUtils.Function_WithExceptions;
...
//                     .collect(toList());
    ...
    private List<URL> convertStringToUrls(String stringOfUrls) {
    	...
    	livelessons.utils.ExceptionUtils.Function_WithExceptions<String,URL> func = null;
    	
        // Create a Function that returns a new URL object when
        // applied.
        Function<String, URL> urlFactory = ExceptionUtils.rethrowFunction(func);
        
        List<URL> results = new ArrayList<URL>();
        for(Object s : Pattern.compile(",").splitAsStream(stringOfUrls).collect(Collectors.toList())){
        	results.add(urlFactory.apply((String)s));
        }
