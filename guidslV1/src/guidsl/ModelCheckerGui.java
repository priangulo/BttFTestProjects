        ...
        RunSpin.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                try {
                    ...
                    if (!bPromela) {
                        Area.append("Generation of Promela code file... UNSUCCESSFUL\n");
                    }
                } catch (Exception ee) {
                ...
                try {
                    ...
                    int error = 0;
                    while ((spinOutput = spinReader.readLine()) != null) {
                        if ((error = parseOutput(spinOutput)) == -1) {
                            break;
                        }
                        //if(error == -1) break;
                    }
                    if (error == 0) {
                        textComment.append("No errors found in model\n");
                    }
    ...
    void checkModel() {
        ...
        while (currentVar.hasNext()) {
            ...
            if (var.userVisible) {
                userVars.add(var);
                if (debug) {
                    System.out.println(var.name);//TODO: declare boolean debug
                }
            }
        }
            ...
            if (combArr == null) {
                ...
                for (int k = 0; k < prevCombArr.length; k++) {
                    ...
                    while (cleanListIter.hasNext()) {
                        ...
                        if (debug) {
                            System.out.println("cleaning up: " + cleanName);
                        }
                    }
                }
                prevCombArr = null;
    ...
    int parseOutput(String spinOutput) {
        ...
        switch (token[0].charAt(0)) {
            case 'p':
                ...
                break;
            case 'W':
                if (token[0].compareTo("Warning") == 0) {
                    Area.append(spinOutput + "\n");
                }
                break;
            case 'h':
