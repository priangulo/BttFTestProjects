...
public class Gui extends SwingApp {
    ...
    public void initLayout() {
        ...
        toolbar.add( showdeb );
        JButton showModelChecker = new JButton("ModelChecker");
        showModelChecker.setToolTipText("Launch Spin Model Checker");
        showModelChecker.addActionListener(new ActionListener(){
            public void actionPerformed(ActionEvent ae){
                new ModelCheckerGui(current, "Spin Model Checker", false);
            }
        });
        toolbar.add(showModelChecker);
        toolbar.addSeparator();
    ...
    super(); {
     ...
     static void lastly() {
        if ( !Main.debug ) {
            // the following propagates all known selections in the model
            // you select debug mode if the model is inconsistent
