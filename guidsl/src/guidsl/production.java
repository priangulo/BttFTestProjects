...
public class production extends gObj {
	...
	public void traverse(GVisitor v) {
		...
		while (i.hasNext()) {
			...
			try {
				...
				Util.fatalError(e.getMessage() + " " + o.getClass().getName());
			}
			p.visit(v);
		}
	}
	...
	public node makef() {
		node result1 = null;  //dsb added by hand
		boolean errorExit = false; //dsb added by hand
		...
		while (i.hasNext()) {
			...
			default:
				...
				result1 = new bterm(t.name); // return some dummy  //dsb added by hand
				errorExit = true; //dsb added by hand
			}
		}
		if (!errorExit) { //dsb added by hand
			switch (type) {
			case production.norm:
			case production.opt:
				if (cnt > 1) {
					// production iff (or of options) AND atmostone( list of
					// options)
					result1 = new and(new iff(new bterm(name), o), n);
				} else {
					// production (prodname iff onlyPattern)
					// the atmostone(x) is always true
					result1 = new iff(new bterm(name), o);
				}
				break;
			case production.star:
			case production.plus:
				// production iff (predicate associated with patterns)
				result1 = new iff(new bterm(name), n);
				break;
			default:
				Util.fatalError("unrecognizable type: " + type + " for production " + name);
			}
		}
		node result = result1;
		switch (type) {
		...
		result.incompleteMessage = result.incompleteMessage + " in " + name + " panel";
		return result;
	}
	...
	public JComponent draw(int several) {
		...
		boolean flag = false;// indicates whether we've started a new tab this
								// time
		...
		if (size == 1) {// consists of only one pattern
			...
			if (several == 1 && ntd) {// if optional parent has non-optional
										// child
			...
			GroupButtons gp = new GroupButtons(min, max);// form a group of
															// buttons
			...
			while (i.hasNext()) {// iterate through the patterns
				...
				row.setLayout(new FlowLayout(FlowLayout.LEFT));
				AbstractButton ab;
				if (several == 1 || several == 2) { // optional production, or
													// plus production
					JCheckBox cb = new JCheckBox(p.var.disp);
					p.var.userVisible = true; // added *dsb*
					row.add(cb);
				...
				p.setWidget(ab);
				row.add(p.draw(several));// call the draw method of whatever the
											// pattern points to
	...
	/*
	 * Rests production's member variables so that different model inputs can be
	 * handled by guidls at runtime
	 */
